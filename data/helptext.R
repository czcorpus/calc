...
helpGr <- "Při procházení náhodného vzorku rozsáhlé konkordance můžeme identifikovat různý počet skupin (např. významů hledaného slova). Rozhraní <a href='https://kontext.korpus.cz'>KonText</a> umožňuje tyto skupiny <a href='http://wiki.korpus.cz/doku.php/manualy:kontext:konkordance#prace_s_konkordanci'>označit čísly</a>, uložit a informaci o rozdělení do skupin zakódovat do URL konkordance, která slouží jako vstup pro tento modul. <br/>Z informací v URL můžeme pomocí tzv. <em>bootstrapingu</em> zjistit konfidenční intervaly pro každou nalezenou skupinu a vyhodnotit tak, do jaké míry jsou spolehlivé údaje 1) o <em>existenci jednotlivých skupin</em> a 2) o <em>jejich pořadí</em>, a to vždy s&nbsp;ohledem na velikost analyzovaného vzorku vzhledem k celkové velikosti konkordance. Zároveň modul vypočítá pravděpodobnost výskytu skupiny, kterou bysme označili jako marginální (např. menšinového významu), ve zkoumaném vzorku a navrhne minimální velikost vzorku, v němž by se s přijatelnou mírou chyby měla hypotetická marginální skupina spolehlivě vyskytnout."

helpGr_ex <- "Ve vzorku 100 výskytů slova <em>měkký</em> můžeme identifikovat 6 významů, přičemž bychom rádi věděli, zda je lze považovat na takovémto vzorku za doložené a jaké je jejich pořadí podle frekvence. S obojím nám pomůžou konfidenční intervaly (viz graf i tabulka). Máme-li navíc důvodné podezření, že hledané slovo má ještě jeden význam, můžeme odhadnout – pokud si stanovíme, jak marginální asi je (např. 1 % z celkové frekvence slova <em>měkký</em>) – s jakou pravděpodobností se v našem vzorku vyskytne a jak velký vzorek bychom potřebovali, abychom ho v něm spolehlivě našli (při stanovené přijatelné míře chyby)."