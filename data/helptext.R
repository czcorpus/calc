...
helpGr <- "Tato záložka slouží k vyhodnocení skupin identifikovaných v&nbsp;konkordanci v&nbsp;rámci aplikace <a href='https://kontext.korpus.cz'>KonText</a>. Při procházení náhodného vzorku rozsáhlé konkordance můžeme identifikovat různý počet skupin (např. různých významů hledaného slova). Rozhraní KonText umožňuje tyto skupiny <a href='http://wiki.korpus.cz/doku.php/manualy:kontext:konkordance#prace_s_konkordanci'>označit čísly</a> a informaci o rozdělení do skupin zakódovat do URL konkordance. <br/>Pokud URL vložíme do kalkulačky, tento modul nám pomocí konfidenčních intervalů vyhodnotí, do jaké míry jsou spolehlivé údaje jednak o <em>existenci jednotlivých skupin</em> a jednak o <em>jejich pořadí</em>, a to vždy s&nbsp;ohledem na velikost analyzovaného vzorku. Zároveň vypočítá pravděpodobnost výskytu marginální skupiny (např. menšinového významu) ve zkoumaném vzorku a navrhne minimální velikost vzorku, v němž by se s přijatelnou mírou chyby měla hypotetická marginální skupina spolehlivě vyskytnout."
