...
helpSaRe_beza1 <- "Tento modul je určen k vyhodnocování spolehlivosti výsledků získaných manuální analýzou jevu v různém počtu náhodných vzorků (typicky z rozsáhlé konkordance, kterou nelze celou detailně prozkoumat). Obvyklým problémem takové analýzy je to, že nejsme schopni určit, kolik dílčích vzorků a s jakou velikostí je třeba ručně prozkoumat, aby byl náš odhad spolehlivý. Univerzální odpověď na tuto otázku dát nelze, protože to závisí na mnoha okolnostech (zejm. na frekvenci a rozpýtelnosti analyzovaného jevu), můžeme nicméně na základě <em>konfidenčních intervalů</em> odhadnout, v jakém rozmezí se daný jev v celé konkordanci vyskytuje.<br/> Modul nabízí informaci o tom, jak se s postupným přidáváním vzorků zpřesňuje náš odhad (zužuje se konfidenční interval) frekvence sledovaného jevu. Na základě tohoto intervalu můžeme rozhodnout, zda nám taková přesnost stačí, či zda raději prozkoumáme další vzorky, abychom odhad ještě víc zpřesnili. Aplikace nabízí dva způsoby jak výpočet provádět: pomocí"
...
helpGr <- "Při zkoumání náhodného vzorku rozsáhlé konkordance můžeme rozdělit výskyty na různý počet skupin (např. významů hledaného slova). Rozhraní <a href='https://kontext.korpus.cz' target='_blank' class='external'>KonText</a> umožňuje tyto skupiny <a href='http://wiki.korpus.cz/doku.php/manualy:kontext:konkordance#prace_s_konkordanci' target='_blank' class='external'>označit čísly</a>, uložit a informaci o rozdělení do skupin zakódovat do URL konkordance, která slouží jako vstup pro tento modul (pokud KonText nepoužíváte, můžete použít <em>Manuální zadání</em>). <br/>Ze zadaných informací můžeme pomocí tzv. <em>bootstrappingu</em> zjistit konfidenční intervaly pro každou nalezenou skupinu a vyhodnotit, do jaké míry jsou spolehlivé údaje jednak o <em>existenci jednotlivých skupin</em> a jednak o <em>jejich pořadí</em> (vždy s&nbsp;ohledem na velikost analyzovaného vzorku). Zároveň modul vypočítá, jaká je pravděpodobnost alespoň jednoho výskytu skupiny, kterou bysme označili jako marginální (např. menšinového významu), ve zkoumaném vzorku."

helpGr_ex <- "Ve vzorku 100 výskytů slova <em>měkký</em> jsme identifikovali 6 významů. Chceme vědět, zda je lze považovat na takovémto vzorku za doložené a jaké je jejich pořadí podle frekvence. S obojím nám pomůžou konfidenční intervaly (viz graf i tabulka), které naznačují, v jakém rozpětí by se výsledky při opakování analýzy mohly vyskytnout. Máme-li navíc důvodné podezření, že hledané slovo má ještě jeden význam, který se ve vzorku neobjevil, můžeme odhadnout, jak pravděpodobné to je. Pokud si stanovíme, že náš význam představuje minimálně 1 % z celkové frekvence slova <em>měkký</em>, je pravděpodobnost, že se objeví ve vzorku o 100 výskytech, zhruba 63 %."

helpNgrams <- "Při mezijazykovém srovnávání víceslovných jednotek narážíme na problém, že si jednotky stejné délky (2-gramy, 3-gramy atp.) nemusí z důvodu typologické odlišnosti přesně odpovídat. Pro vyrovnání tohoto rozdílu lze vytvořit kvantitativní model, který je založen na ekvivalenci počtu typů (T) mezi srovnávanými jazyky (podrobnosti viz Milička – Cvrček – Lukešová, připravuje se). Jeho vstupními parametry jsou <em>n</em> (=délka n-gramu) a <em>t</em> (=frekvenční minimum), protože to jsou hlavní faktory, které počet typů můžou ovlivnit. Tyto parametry lze pro každý jazykový pár modifikovat koeficienty <em>a</em> a <em>b</em>. Aplikací těchto koeficientů v modelu zjistíme pro daný pár jazyků, jaké hodnoty <em>n</em> a <em>t</em> si odpovídají (viz schématické znázornění pomocí interpolace v grafu).<br/> Zvýšení či snížení frekvenčí hladiny <em>t</em> je většinou při extrakci dat z korpusu jednoduché a poměrně intuitivní, problémy může činit hodnota <em>n</em>, která po transformaci nemusí být celé číslo (např. 2.34-gram). Pokud ovšem hodnotu <em>n</em> chápame jako průměr v nějakém vzorku, lze desetinné hodnoty tohoto parametru dosáhnout smícháním n-gramů různé délky (viz box pod grafem)."

helpNgrams_ex <- "Chceme-li najít korespondenci mezi bigramy v češtině s minimální frekvencí 10 a n-gramy v němčině, musíme použít příslušné předpočítané koeficienty <em>a</em> a <em>b</em>. S jejich pomocí zjistíme, že českým bigramům odpovídají německé 2.34-gramy s minimální frekvencí 10.3. Tuto korespondenci schématicky zachycuje graf počtu typů (T) v obou jazycích, naznačující zároveň, jak vypadala originální data pro němčinu a jak vypadají po transformaci. Box pod grafem pak ukazuje, v jakém poměru namíchat 2-gramy a 3-gramy, abychom ve výsledku dostali průměrnou délku rovnající se transformovanému <em>n</em>."
